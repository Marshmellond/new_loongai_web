...
const push_func = () => {
  counter.file_ai_content = ""
  counter.file_ai_content_markdown = ""
  counter.file_show_load_status = true
  const url = "/api/file/generate/openai"
  let body = {
    content: counter.file_inp_data.content,
    name: counter.file_inp_data.name,
    type: counter.file_inp_data.type,
  }
  fetch(url, {
    method: "POST",
    ...
    if (res.ok) {
      temp_meg.value = ""
      counter.file_chat_status_bool = true
      counter.file_show_load_status = false
      setTimeout(() => {
        Prism.highlightAll()
      }, 100)
      return res.body;
    ...
    function process({done, value}) {
      ...
      if (counter.file_chat_status_bool === false) {
        return;
      }
      ...
      counter.file_ai_content_markdown = marked(temp_meg.value)
      counter.file_ai_content = temp_meg.value
...
const on_generate = () => {
  if (counter.file_inp_data.content.length === 0) {
    message.error("文件未上传")
  } else {
    push_func()
  }
...
const uploadFileToServer2 = (fileContent, fileName, fileType) => {
  counter.file_inp_data.content = fileContent
  counter.file_inp_data.name = fileName
  counter.file_inp_data.type = fileType
  console.log(fileContent, fileName, fileType)
};
const delete_put_file = () => {
  counter.file_inp_data.content = ""
  counter.file_inp_data.name = ""
  counter.file_inp_data.type = ""
}
</script>

<template>
  <div class="div-home">文件内容识别</div>
  <div class="div1">
    <div class="div-title"
    ...
    <div class="div7-content" v-if="counter.file_inp_data.content.length!==0">
      <div class="div7-title">
        ...
        <span style="margin-left: 0.2vw" class="div-7-span">{{ counter.file_inp_data.name }}</span>
      </div>
  ...
  <div class="div2">
    <a-space direction="vertical" class="ant-space">
      <a-button type="primary" size="large" class="ant-button1" @click="on_generate">
        ...
        <span style="color: black">识别</span>
      </a-button>
    </a-space>
  </div>
</template>
